...
class MovieTopRatedFragment : BaseFragment<FragmentHomeBinding>(R.layout.fragment_home) {

    private val movieTopRatedViewModel: MovieTopRatedViewModel by viewModel()
    ...
    private fun init() {
        binding.recyclerViewMovie.apply {
            adapter = this@MovieTopRatedFragment.adapter
            layoutManager = LinearLayoutManager(context)
        }
    }

    private fun subscribeUi() {
        movieTopRatedViewModel.movie.observe(viewLifecycleOwner, {
            adapter.submitList(it)
        })

        movieTopRatedViewModel.getLoading().observe(viewLifecycleOwner, { loading ->
    ...
    private fun navigationToDetailMovie(movie: Movie) {
        ...
        findNavController().navigate(
            MovieTopRatedFragmentDirections.actionMovieTopRatedFragmentToMovieDetailFragment(
                movie,
                movie.title ?: ""
            ), options
        )
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.favorite -> navigateToFavorite(MovieTopRatedFragmentDirections.actionMovieTopRatedFragmentToFavoriteFragment())
        }
