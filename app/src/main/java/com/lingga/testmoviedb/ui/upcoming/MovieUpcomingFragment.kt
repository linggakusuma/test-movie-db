...
class MovieUpcomingFragment : BaseFragment<FragmentHomeBinding>(R.layout.fragment_home) {
    ...
    private fun init() {
        binding.recyclerViewMovie.apply {
            adapter = this@MovieUpcomingFragment.adapter
    ...
    private fun subscribeUi() {
        upcomingViewModel.movie.observe(viewLifecycleOwner, {
            adapter.submitList(it)
        })

        upcomingViewModel.getLoading().observe(viewLifecycleOwner, { loading ->
    ...
    private fun navigationToDetailMovie(movie: Movie) {
        ...
        findNavController().navigate(
            MovieUpcomingFragmentDirections.actionMovieUpcomingFragmentToMovieDetailFragment(
                movie,
                movie.title ?: ""
            ), options
        )
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.favorite -> navigateToFavorite(MovieUpcomingFragmentDirections.actionMovieUpcomingFragmentToFavoriteFragment())
        }
