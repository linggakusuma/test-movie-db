...
class MovieNowPlayingFragment : BaseFragment<FragmentHomeBinding>(R.layout.fragment_home) {

    private val movieNowPlayingViewModel: MovieNowPlayingViewModel by viewModel()
    ...
    private fun init() {
        binding.recyclerViewMovie.apply {
            adapter = this@MovieNowPlayingFragment.adapter
            layoutManager = LinearLayoutManager(context)
        }
    }

    private fun subscribeUi() {
        movieNowPlayingViewModel.movie.observe(viewLifecycleOwner, {
            adapter.submitList(it)
        })

        movieNowPlayingViewModel.getLoading().observe(viewLifecycleOwner, { loading ->
    ...
    private fun navigationToDetailMovie(movie: Movie) {
        ...
        findNavController().navigate(
            MovieNowPlayingFragmentDirections.actionMovieNowPlayingFragmentToMovieDetailFragment(
                movie,
                movie.title ?: ""
            ), options
        )
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.favorite -> navigateToFavorite(MovieNowPlayingFragmentDirections.actionMovieNowPlayingFragmentToFavoriteFragment())
        }
